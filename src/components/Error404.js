...
export default function Error404() {
  ...
  // ── Escuchar conexión en todo momento ──
  useEffect(() => {
    const handleOnline = () => {
      setOnline(true);
      setRetrying(true);
  ...
  // ── Polling para detectar que vuelve el servidor ──
  useEffect(() => {
    ...
    <div className="error-page">
      ...
      <div className="error-hero">
        ...
        <div className="error-hero-content">
          <span className="error-eyebrow">Error de conexión</span>
          <h1 className="error-title">No pudimos cargar la página</h1>
          <p className="error-subtitle">
            {online
              ? retrying
                ? "Conexión restaurada, recargando..."
                : "El servidor no responde. Estamos reintentando cada 5 segundos."
              : "Sin conexión a internet. Revisá tu red."}
          </p>
          <div className="error-status">
            <span className={`status-dot ${online ? "online" : "offline"}`} />
            <span className="status-label">
              {online ? (retrying ? "Reconectando..." : "En línea · esperando servidor") : "Sin conexión"}
            </span>
          </div>
          <button
            className="btn-hero"
        ...
        <div className="game-header">
          <div>
            ...
            <p className="game-desc">
              Hay <strong>{ROBOT_COUNT} robots</strong> escondidos en la grilla. Hacé clic en las celdas seguras para sumar puntos. Llegá a <strong>{WIN_SCORE} puntos</strong> sin encontrar todos los robots para ganar. Clic derecho para marcar una celda.
            </p>
          </div>
